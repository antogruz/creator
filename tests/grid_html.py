...
class GridGeneratorTester(Tester):
    ...
    def one_quad_test(self):
        assert generate_grid_html(1, 1) == '<div class="line-0 col-0 w-1 h-1">0</div>'

    def two_lines_test(self):
        html = generate_grid_html(2, 1)
        assert html == '<div class="line-0 col-0 w-1 h-1">0</div><div class="line-1 col-0 w-1 h-1">1</div>'

    def two_columns_test(self):
        html = generate_grid_html(1, 2)
        assert html == '<div class="line-0 col-0 w-1 h-1">0</div><div class="line-0 col-1 w-1 h-1">1</div>'
